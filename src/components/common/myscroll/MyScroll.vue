<template>
  ...
  >
    ...
    >
      ...
      <div v-if="state == 4">
        <!-- <img v-if="IsActive == true" src="~assets/img/loading/Load2.jpg" alt="" 
        class="imgLoad" ref='imgLoad' > -->
      </div>
      <div class="scroll-bottom" ref="bottom">
...
import Load from "assets/img/loading/Load.gif";
export default {
  name: "MyScroll",
  props: {
    page: {
  ...
  methods: {
    ...
    touchMove(e) {
      ...
      if (this.scrollState && e.targetTouches[0].pageY > this.pageY) {
        ...
        if (this.myScroll.scrollTop == 0) {
          ...
          let ranget = (diff / document.body.clientHeight) * 400; //计算在屏幕上滑动了多少
          if (ranget > 20) {
            this.state = 2;
          } else if (ranget < 15) {
            this.state = 6;
      ...
      if(e.targetTouches[0].pageY < this.pageY){
         this.aspect = 1; //判断是否上拉
         if(this.pageY - e.targetTouches[0].pageY <150) { //判断滑动距离是否太大
            this.state = 4;
         }
      }
    },
    touchEnd(e) {
      ...
      if(this.aspect == 1 && this.state == 4) {
        if(this.myScroll.offsetHeight - this.pageY < 200) {
          this.bottomCallback();
